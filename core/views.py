...
def _admin_dashboard(request):
    ...
    upcoming_qs = Semester.objects.filter(due_date__gte=today_date, due_date__lte=today_date + td(days=90), is_active=True).order_by('due_date')

    seen_overdue = set()
    overdue_names = []
    for s in overdue_qs:
        if s.name not in seen_overdue:
            seen_overdue.add(s.name)
            overdue_names.append(s.name)

    seen_upcoming = set()
    upcoming_list = []
    for s in upcoming_qs:
        if s.name not in seen_upcoming:
            seen_upcoming.add(s.name)
    ...
    return render(request, 'dashboard.html', {
        ...
        'uni_labels': json.dumps(uni_labels),
        'uni_values': json.dumps(uni_values),
        ...
        'enquiry_status_labels': json.dumps(enquiry_status_labels),
        'enquiry_status_values': json.dumps(enquiry_status_values),
        'student_status_labels': json.dumps(student_status_labels),
        'student_status_values': json.dumps(student_status_values),
        'fee_monthly_data': json.dumps(fee_monthly_data),
        'counsellor_labels': json.dumps(counsellor_labels),
        'counsellor_values': json.dumps(counsellor_values),
...
def _accountant_dashboard(request):
    ...
    return render(request, 'dashboard_accountant.html', {
        ...
        'chart_modes': json.dumps(chart_modes),
        'chart_mode_amounts': json.dumps(chart_mode_amounts),
        'chart_unis': json.dumps(chart_unis),
        'chart_uni_amounts': json.dumps(chart_uni_amounts),
        'attendance': attendance,
        'today_date': today_date,
    })
